...
真实流程不使用本地地点目录、Mock 路线、默认天气或默认当前位置；`local` 和 `live` 请求都必须经过真实服务与验证，不能用示例状态绕过。

离线评测使用中性的测试地点目录和确定性测试 Planner；它们只用于校验器与 Agent 循环测试，不提供真实用户入口。真实结果页拒绝显示未经过真实数据确认的方案。

## 15. 当前限制

- 高德真实流程面向中国境内，地点解析失败时必须补问或要求用户选择候选。
