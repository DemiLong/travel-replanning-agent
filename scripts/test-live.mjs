...
const regressionText="我十点要去故宫，现在已经11:46了，下午3点需要和朋友在酒店集合，下午5点预约了景点参观，航班晚点了。";
...
try{
  ...
  if(!process.argv.includes("--semantic-only")){
    ...
    const result=await replanReal(input);assert("ok" in result);assert(result.candidateComparisons?.length>=1&&result.candidateComparisons.length<=2);assert(result.attempts.length<=2);
