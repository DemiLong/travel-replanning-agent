...
export class AmapRoutesService {
  ...
  async route(origin: RouteEndpoint, destination: RouteEndpoint, travelMode: TravelMode, signal?: AbortSignal): Promise<WorldRoute> {
    requireAmapKey();
    origin = { ...RouteEndpointSchema.parse(origin), ...await this.coordinates.toGCJ02(origin, signal) };
    destination = { ...RouteEndpointSchema.parse(destination), ...await this.coordinates.toGCJ02(destination, signal) };
    ...
    try {
      ...
      const body = await amapGet(path, { origin: await this.coordinates.format(origin, signal), destination: await this.coordinates.format(destination, signal), ...(travelMode === "TRANSIT" ? { city: origin.city, cityd: destination.city, strategy:"0", extensions:"base" } : { extensions:"base" }) }, 120000, { signal });
  ...
  async batch(pairs: Array<{origin:RouteEndpoint;destination:RouteEndpoint}>, mode: TravelMode, signal?: AbortSignal): Promise<WorldRoute[]> {
    ...
    for (let i=0;i<unique.length;i+=3) output.push(...await Promise.all(unique.slice(i,i+3).map(p=>this.route(p.origin,p.destination,mode,signal))));
    return output;
  }
}
