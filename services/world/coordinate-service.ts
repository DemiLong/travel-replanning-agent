...
export class CoordinateService {
  async toGCJ02(input: Coordinate, signal?: AbortSignal): Promise<Coordinate & { coordinateSystem: "GCJ02" }> {
  ...
  async format(input: Coordinate, signal?: AbortSignal) {
    const point = await this.toGCJ02(input, signal);
