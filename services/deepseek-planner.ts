...
For each inbound leg select travelMode from available grounded routes, respecting explicit restrictions. Compare viable modes without asking the traveler by default. Consider walking tolerance, time pressure and known fares; unknown taxi fare is not zero. DRIVING is a taxi recommendation unless the traveler explicitly has a car. Never invent travel times. Existing activities with durationSource=unknown and locked=false require a proposed durationMinutes (10–180), labelled as a suggestion. A locked activity with durationSource=unknown has only a fixed arrival time: retain it as the final step, durationMinutes=null, never schedule anything after it or claim its end time is known. endTime=startTime on these inputs is an internal unknown-duration marker, not a completed activity.
...
export function materializeCandidate(c:AgentContext,raw:PlanCandidate):ProposedPlan{
  ...
  for(const [index,step] of candidate.steps.entries()){
    ...
    const anchor=old?.locked&&old.durationSource==="unknown";
    if(anchor&&index!==candidate.steps.length-1)throw new Error("固定预约结束时间未知，不能据此安排后续活动。");
    const duration=anchor?0:old&&old.durationSource!=="unknown"?minutes(old.endTime)-minutes(old.startTime):step.durationMinutes;
    if(duration===null||duration===undefined||duration<0||(!anchor&&duration===0))throw new Error("活动时长缺失或无效。");
    ...
    if(old?.durationSource==="unknown"&&!old.locked){created.durationSource="suggested";created.constraint+="；停留时长为本方案建议，尚非用户提供事实。";}
