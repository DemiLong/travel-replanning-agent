...
export class DeepSeekPlanner implements CandidatePlanner {
  ...
  async generateCandidates(context:AgentContext,feedback:Violation[],attempt:number,signal?:AbortSignal){
    if(context.world?.status!=="ready")throw new Error("WORLD_CONTEXT_NOT_READY");
      const response=await this.client.responses.parse({model:this.name,store:false,reasoning:{effort:"none"},max_output_tokens:5000,input:[{role:"system",content:DEEPSEEK_PLANNER_PROMPT},{role:"user",content:JSON.stringify({confirmedItinerary:context.remainingEvents,impactAnalysis:context.impactAnalysis,userRequest:context.disruption,preferences:context.profile,world:context.world,validationFeedback:feedback,attempt})}],text:{format:deepSeekFormat(CandidateSetSchema,"coveredYou_plan_candidates")}}, { signal });
...
export function materializeCandidate(c:AgentContext,raw:PlanCandidate):ProposedPlan{
  ...
  for(const [index,step] of candidate.steps.entries()){
    ...
    events.push(old?{...old,startTime:time(start),endTime:time(end),status:old.locked?"locked":"planned",travelTimeFromPrevious:transfer,reason:step.reason,openingTime:null,closingTime:null}:{id:`ai-${poi!.poiId}-${index}`,placeId:id,name:poi!.name,category:poi!.type,startTime:time(start),endTime:time(end),location:poi!.address||poi!.name,status:"planned",locked:false,indoorOutdoor:"mixed",openingTime:null,closingTime:null,travelTimeFromPrevious:transfer,reason:step.reason,constraint:"候选停留时长为建议；营业状态与室内外情况尚未核实。"});
