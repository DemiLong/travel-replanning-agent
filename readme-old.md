...
coveredYou 只做一件事：当用户描述今天接下来的安排，并遇到晚点、天气、体力、地点关闭或明确想重排时，基于真实地点与路线数据给出可接受的新方案。已有正式行程可以直接调整；空会话也可以从本次输入建立草稿并补齐必要字段。它不负责账号、云同步、多日、海外、预订或长期推荐。
...
- `/`：输入本次明确变化和接下来的安排；无预存行程时也可建立本次草稿，一次只补充一个必要字段。
